...
class DevelopmentalPrimitivePerceptionFrameDiff:
    ...
    added_objects: ImmutableSet["ObjectPerception"] = attrib(kw_only=True, converter=_to_immutableset,
                                                             default=immutableset())
    r"""
    the set of `ObjectPerception`\ s, that were present on the second perception frame, but not the first.
    """
    removed_objects: ImmutableSet["ObjectPerception"] = attrib(kw_only=True, converter=_to_immutableset,
                                                               default=immutableset())
    r"""
    the set of `ObjectPerception`\ s, that were present on the first perception frame, but not the second.
    """
    added_property_assertions: ImmutableSet["PropertyPerception"] = attrib(kw_only=True, converter=_to_immutableset,
                                                                           default=immutableset())
    r"""
    the set of `PropertyPerception`\ s, that were present on the second perception frame, but not the first.
    """
    removed_property_assertions: ImmutableSet["PropertyPerception"] = attrib(kw_only=True, converter=_to_immutableset,
                                                                             default=immutableset())
    r"""
    the set of `PropertyPerception`\ s, that were present on the first perception frame, but not the second.
    """
    added_relations: ImmutableSet["RelationPerception"] = attrib(kw_only=True, converter=_to_immutableset,
                                                                 default=immutableset())
    r"""
    the set of `RelationPerception`\ s, that were present on the second perception frame, but not the first.
    """
    removed_relations: ImmutableSet["RelationPerception"] = attrib(kw_only=True, converter=_to_immutableset,
                                                                   default=immutableset())
    r"""
...
def diff_primitive_perception_frames(before: DevelopmentalPrimitivePerceptionFrame,
    ...
    return DevelopmentalPrimitivePerceptionFrameDiff(
        ...
        added_property_assertions = after.property_assertions.difference(before.property_assertions),
        removed_property_assertions = before.property_assertions.difference(after.property_assertions),
