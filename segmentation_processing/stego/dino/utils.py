...
def load_pretrained_weights(model, pretrained_weights, checkpoint_key, model_name, patch_size):
    if os.path.isfile(pretrained_weights):
        ...
        if url is not None:
            ...
            print(
                "There is no reference weights available for this stego_model => We use random weights."
            )
...
def restart_from_checkpoint(ckp_path, run_variables=None, **kwargs):
    ...
    # example: {'state_dict': stego_model}
...
def bool_flag(s):
    ...
    """
    FALSY_STRINGS = {"off", "false", "0"}  # pylint: disable=invalid-name
    TRUTHY_STRINGS = {"on", "true", "1"}  # pylint: disable=invalid-name
