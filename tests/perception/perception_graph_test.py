...
from adam.ontology.phase1_ontology import BIRD, TABLE, TRUCK, \
    _TRUCK_SCHEMA, \
    above, bigger_than, on
from adam.perception.high_level_semantics_situation_to_developmental_primitive_perception import \
    GAILA_PHASE_1_PERCEPTION_GENERATOR
from adam.perception.perception_graph import PerceptionGraphPattern, to_perception_graph
from adam.random_utils import RandomChooser
from adam.situation.templates.phase1_templates import Phase1SituationTemplate, color_variable, \
    object_variable
...
def test_match_truck_and_table():
    ...
    truck = object_variable(debug_handle="truck_0",
                            root_node=TRUCK,
    ...
    situations_with_truck = chain(all_possible_test(truck_on_table_template),
    ...
    for (idx, situation_with_truck) in enumerate(situations_with_truck):
        perception = GAILA_PHASE_1_PERCEPTION_GENERATOR.generate_perception(situation_with_truck,
    ...
    bird = object_variable(debug_handle="bird_0",
                            root_node=BIRD,
                            added_properties=[color])
    ...
    situations_with_bird = chain(all_possible_test(bird_on_table_template),
    ...
    for situation_with_bird in situations_with_bird:
        perception = GAILA_PHASE_1_PERCEPTION_GENERATOR.generate_perception(situation_with_bird)
        perception_graph = to_perception_graph(perception.frames[0])
