...
from adam.ontology.phase1_spatial_relations import Region, Direction
...
from adam.perception.developmental_primitive_perception import \
    DevelopmentalPrimitivePerceptionFrame, HasBinaryProperty, HasColor, RgbColorPerception
...
class PerceptionGraph:
    ...
    def _to_dot_node(self, dot_graph: graphviz.Digraph,
                                               perception_node: PerceptionGraphNode,
                     next_node_id: Incrementer) -> str:
        ...
        # regions do have content but we express those as edges to other nodes
            ...
            return str(perception_node.cross_section) + str(perception_node.cross_section_size)
        else:
            raise RuntimeError(f"Do not know how to perception node render node "
...
def _add_labelled_edge(graph: DiGraph, source: Any, target: Any, unmapped_label: Any,
                       *, map_edge: _EdgeMapper):
    graph.add_edge(source, target)
    graph.edges[source, target].update(map_edge(unmapped_label))


def _translate_axes(graph: DiGraph, owner: HasAxes,
                    mapped_owner: Any,
    ...
    _add_labelled_edge(graph, owner, mapped_primary_axis, PRIMARY_AXIS_LABEL,
                       map_edge=map_edge)
    graph.add_nodes_from(map_axis(orienting_axis) for orienting_axis in
                         owner.axes.orienting_axes)
    for orienting_axis in owner.axes.orienting_axes:
        mapped_axis = map_axis(orienting_axis)
        _add_labelled_edge(graph, mapped_owner, mapped_axis, HAS_AXIS_LABEL,
                           map_edge=map_edge)
    ...
    for axis_relation in owner.axes.axis_relations:
        ...
        _add_labelled_edge(graph, mapped_arg1, mapped_arg2, axis_relation.relation_type,
                           map_edge=map_edge)
...
class PerceptionGraphPattern:
    ...
    def matcher(self, graph_to_match_against: PerceptionGraph) -> "PerceptionGraphPatternMatching":
    ...
    def from_schema(object_schema: ObjectStructuralSchema) -> "PerceptionGraphPattern":
        ...
        PerceptionGraphPattern._translate_schema(
            ...
            object_=object_schema, object_schema=object_schema, graph=graph,
    ...
    def _translate_schema(
            ...
            object_schema: ObjectStructuralSchema,
                          graph: DiGraph,
                          object_to_node: Dict[Any, NodePredicate]) -> None:
        ...
        def map_node(node: Any) -> NodePredicate:
            if node not in schema_node_to_pattern_node:
                if isinstance(node, GeonAxis):
                    ...
                    schema_node_to_pattern_node[node] = GeonPredicate.exactly_matching(node)
                elif isinstance(node, Region):
                    # TODO: double-check other aspects of region are handled by edges
                    schema_node_to_pattern_node[node] = RegionPredicate.matching_direction(node)
                else:
        ...
        object_graph_node = AnyObjectPerception()
        graph.add_node(object_graph_node)
        object_to_node[object_] = object_graph_node

        _translate_axes(graph, object_schema, object_graph_node, map_node,
                        map_edge)
        _translate_geon(graph, object_schema, mapped_owner=object_graph_node, map_geon=map_node,
                        map_axis=map_node, map_edge=map_edge)

        regions = immutableset(relation.second_slot for relation in object_schema.sub_object_relations
                               if isinstance(relation.second_slot, Region))

        for region in regions:
            _translate_region(graph, region, map_node=map_node,
                              map_edge=map_edge)
        ...
        for sub_object in object_schema.sub_objects:
            PerceptionGraphPattern._translate_schema(sub_object, sub_object.schema,
                                                     graph,
                                                     object_to_node)
            graph.add_edge(object_to_node[sub_object],
                           object_graph_node, predicate=RelationTypeIsPredicate(PART_OF))
        ...
        for sub_object_relation in object_schema.sub_object_relations:
            if sub_object_relation.negated:
                raise RuntimeError("Did not expect a negated relation in a sub-object schema")
            graph.add_edge(object_to_node[sub_object_relation.first_slot],
                object_to_node[sub_object_relation.second_slot],
    ...
    def render_to_file(self, title: str, output_file: Path) -> None:
        ...
        pattern_nodes_to_dot_node_ids = {
            pattern_node : to_dot_node(pattern_node)
            for pattern_node in self._graph.nodes
        }
        ...
        for (source_node, target_node, predicate) in self._graph.edges.data("predicate"):
            ...
            if False:
                dot_graph.edge(source_dot_node, target_dot_node,
                           predicate.dot_label())
            else:
                dot_graph.edge(target_dot_node, source_dot_node,
                               predicate.dot_label(), back="true")
...
class AxisPredicate(NodePredicate):
    ...
    aligned_to_gravitational: Optional[bool] = attrib(validator=optional(instance_of(bool)))
    ...
    def __call__(self, object_perception: PerceptionGraphNode) -> bool:
        if isinstance(object_perception, GeonAxis):
            ...
            if self.directed is not None and self.directed != object_perception.directed:
                return False
            if self.aligned_to_gravitational is not None and self.aligned_to_gravitational != \
...
class PerceptionGraphPatternMatch:
    matched_pattern: PerceptionGraphPattern = attrib(validator=instance_of(PerceptionGraphPattern), kw_only=True)
    graph_matched_against: PerceptionGraph = attrib(validator=instance_of(PerceptionGraph),
                                                    kw_only=True)
    matched_sub_graph: PerceptionGraph = attrib(validator=instance_of(PerceptionGraph), kw_only=True)
...
class PerceptionGraphPatternMatching:
    pattern: PerceptionGraphPattern = attrib(validator=instance_of(PerceptionGraphPattern))
    graph_to_match_against: PerceptionGraph = attrib(validator=instance_of(PerceptionGraph))
...
def _translate_geon(graph: DiGraph, owner: MaybeHasGeon,
                    *,
                    mapped_owner: Any,
                    map_geon: Callable[[Geon], Any],
                    map_axis: _AxisMapper,
                    map_edge: _EdgeMapper) -> None:
    if owner.geon:
        mapped_geon = map_geon(owner.geon)
        graph.add_node(mapped_geon)
        _add_labelled_edge(graph, mapped_owner, mapped_geon, HAS_AXIS_LABEL,
                           map_edge=map_edge)
        _translate_axes(graph, owner.geon, mapped_geon, map_axis,
                        map_edge)
        mapped_generating_axis = map_axis(owner.geon.generating_axis)
        _add_labelled_edge(graph, mapped_geon, mapped_generating_axis, GENERATING_AXIS_LABEL,
                           map_edge=map_edge)


def _translate_region(graph: DiGraph, region: Region[Any], *,
                      map_node: Callable, map_edge: _EdgeMapper,
                      axes_info: Optional[AxesInfo] = None) -> None:
    mapped_region = map_node(region)
    mapped_reference_object = map_node(region.reference_object)
    _add_labelled_edge(graph, mapped_region, mapped_reference_object, REFERENCE_OBJECT_LABEL,
                       map_edge=map_edge)
    if region.direction:
        ...
        _add_labelled_edge(graph, mapped_region, mapped_axis_relative_to,
                           region.direction, map_edge=map_edge)
...
def to_perception_graph(frame: DevelopmentalPrimitivePerceptionFrame) -> PerceptionGraph:
    ...
    for perceived_object in frame.perceived_objects:
        ...
        _translate_axes(graph, perceived_object, perceived_object,
                        lambda axis: axis,
                        lambda label: label)
        _translate_geon(graph, perceived_object, mapped_owner=perceived_object,
                        map_geon=lambda geon: geon,
                        map_axis=lambda axis: axis,
                        map_edge=map_edge)

    regions = immutableset(relation.second_slot for relation in frame.relations
                           if isinstance(relation.second_slot, Region))

    for region in regions:
        _translate_region(graph, region, map_node=lambda x:x ,
                          map_edge=map_edge, axes_info=frame.axis_info)

    # Every relation is handled as a directed graph edge
    # from the first argument to the second
    for relation in frame.relations:
        graph.add_edge(relation.first_slot, relation.second_slot, label=relation.relation_type)
...
class AndPredicate(NodePredicate):
    ...
    def __call__(self, object_perception: PerceptionGraphNode) -> bool:
        return all(sub_predicate(object_perception) for sub_predicate in self.sub_predicates)
...
class EdgePredicate(ABC):
    @abstractmethod
    def __call__(self, source_object_perception: PerceptionGraphNode,
                 edge_label: PerceptionGraphEdgeLabel,
                 dest_object_percption: PerceptionGraphNode) -> bool:
        """
...
def any_edge(source_object_perception: ObjectPerception, edge_label: Any,
             dest_object_perception: ObjectPerception) -> bool:
    return True

def relation_of_type(relation_type: OntologyNode) -> EdgePredicate:
    def func(source_object_perception: ObjectPerception, edge_label: Any,
