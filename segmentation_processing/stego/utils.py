...
def load_model(model_type, data_dir):
    if model_type == "robust_resnet50":
        ...
        model_weights_modified = {
            name.split("stego_model.")[1]: value
            for name, value in model_weights["stego_model"].items()
            if "stego_model" in name
        }
        ...
        model_weights = torch.load(model_file)
        # model_weights_modified = {name.split('stego_model.')[1]: value for name, value in model_weights['stego_model'].items() if
        #                          'stego_model' in name}
...
def get_unnorm():
    return UnNormalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
...
class UnsupervisedMetrics(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        with torch.no_grad():
            ...
            mask = (
                (actual >= 0) & (actual < self.n_classes) & (preds >= 0) & (preds < self.n_classes)
            )
            actual = actual[mask]
            preds = preds[mask]
            self.stats += (
    ...
    def map_clusters(self, clusters):
        if self.extra_clusters == 0:
            ...
            missing = sorted(
                list(set(range(self.n_classes + self.extra_clusters)) - set(self.assignments[0]))
            )
